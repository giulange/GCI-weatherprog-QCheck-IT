...
\newcommand{\gci}{\update{MyNameIsBeautiful}\xspace}
...
One example working in USA using already checked daily data, is given by the expert-based model called PRISM developed by  \cite{Daly08_PRISM_USA}. (\ldots search for other REFs.)
...
In this paper a strongly modular Geospatial Cyber-Infrastructure (GCI), called MyNameIsBeautiful
(two options: (a) AgriProg, because it can (it's ready to) embed the whole agroecological system; (b) WeatherProg-GCI, per differenziarlo dal WeatherProg motore agrometeo)
is proposed as a general technological framework being a WB-SDSS, that can be fully tailored to the specific needs of an existent agrometeorological network.
...
\section{The cyber-physical infrastructure \gci}[Raffaele][done]
...
\gci is designed to be a comprehensive framework for agrometereological monitoring and data management. It can be effectively used for a wide range of purposes, from simple agrometeorogical monitoring to digital maps production and predictive modeling. Clearly, different real-world applications have different requirements, which are all accomodated by \gci thanks to its strongly modular architecture. Components can be added or removed to a specific instance of \gci effortlessly without affecting other parts of the system, as well as modules of the core engine, WeatherProg, which can be selectively enabled or disabled. This way, end users actually derive their own specific SDSS, from the general model of \gci.

The high-level complete architecture of \gci is illustrated in \cref{cyberPhysicalSystemFig}. At the topmost level, \gci is constituted by central components, responsible for storage and processing of data, and peripheral components, in charge of collecting data from the environment.
...
Peripheral components are the actual monitoring networks. In fact, \gci is capable of handling and integrating data coming from multiple heterogeneous networks of stations, possibly under the responsibility of different agencies. Each network can include different kinds of station, and each network can submit data to \gci in different formats, since the decoding module of WeatherProg can be configured to accomodate such differences.

\gci can also be employed in contexts where there is insufficient agroclimatic coverage of the environment. In fact, \gci comes with its own monitoring network, characterised by low-cost stations which are particularly suitable for temporary deployments and for tuning the parameters of the various models included in the GCI.  \note{[Martino, Langella, Nicolazzo 2019]}. It must be stressed that the monitoring network of \gci is entirely optional, being a component like the others that can be simply removed if there is already excellent coverage of the territory, therefore not preventing the use of \gci in such scenarios.

The main central components are the WeatherProg core engine, and the data bases. Two main data bases are employed by \gci. The first is a relational data base used to store the climatic data of the station in a WMO-compliant fashion. The latter  is a raster data base capable of handling the digital climatic maps which can be produced by the WeatherProg engine. The rasdaman DBMS \citep{baumann:rasdaman} has been chosen due to its representation of spatial data as datacubes, i.e. digital maps with more than two dimensions, where the third dimension is used to represent time.

\note{Are pest risk models included into WeatherProg now?}

\note{Should we mention deep learning and other post-processing engines?}
...
\subsection{\note{[MOVE TO LCN]} Monitoring with Low-Cost Station}[Raffaele][todo]
...
\subsection{Data base}[Raffaele][update]
\note{Here we have only the relational database, but we have mentioned earlier the raster data base. Should we mention that the latter has not yet developed or do we have something?}
...
\begin{description}
	\item[UNPROCESSED] The measurement has not been subject to the quality control. \note{This flag no longer makes sense with the addition of the overall quality control flag, and should not be mentioned if not removed altogether}
	...
	\item[MISSING] This measurement is not present in the database \note{This flag no longer makes sense with the current version of the database, and should not be mentioned if not removed altogether}
	\item[INVERTED] This measurement of a maximum (minimum) value of a climatic variable appears to be inverted with its respective minimum {maximum} \note{This flag no longer makes sense with the current version of the database, and should not be mentioned if not removed altogether}
\end{description}
...
\subsection{WeatherProg (general overview)}[Giuliano]
...
\section{Case study: the Campania Region}[Giuliano][todo]

\subsection{Study area}[Giuliano]
...
\begin{table}[]
    ...
    \begin{tabular}{c|c}
        ...
        Leaf wetness & 23
    \end{tabular}
    \caption{Detail of the agrometereological network}
...
\subsection{Weatherprog (specific emphasis on QCheck)}[Giuliano]
...
\subsection{\note{[MOVE TO LCN]} Integration of the monitoring networks\label{Integration}}[Giuliano]
